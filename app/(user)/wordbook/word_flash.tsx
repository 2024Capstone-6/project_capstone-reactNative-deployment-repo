...
export default function WordFlashScreen() {
  const { id, level, words: wordsString, type } = useLocalSearchParams();
  const words = JSON.parse(wordsString as string);
  ...
  return (
    <View className="flex-1">
      <StudyCard words={words} grammars={[]} type={type as '단어'} onComplete={handleComplete} />
    </View>
  );
}
